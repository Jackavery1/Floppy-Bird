...
export function buildMenuToggleButton(scene, elements, cfg) {
    ...
    const visualH = cfg.height ?? MIN_TOUCH;
    const hitH = cfg.hitHeight ?? visualH;
    const touchW = Math.max(cfg.width, MIN_TOUCH);
    const bg = scene.add.rectangle(cfg.cx, cfg.cy, cfg.width, visualH, cfg.color, 0.88);
